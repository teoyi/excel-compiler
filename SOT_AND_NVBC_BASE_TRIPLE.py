...
FILE_PATH_NVBC = "C:\\Users\\yipen\\Desktop\\compiled_NVBC.xlsx"
...
sot_base_writer = pd.ExcelWriter(
    "C:\\Users\\yipen\\Desktop\\SOT_BASE_COMPILE.xlsx", engine="xlsxwriter"
)
...
def data_fill(df):
    for i in df.index:
        ...
        if (
            ...
            for col in required_headers:
                if col == "Driver Team":
                    ...
                    df.at[i, col] = (
                        ...
                        + ","
                        + str(sot_match.iloc[0][col])
                        + ","
                        + str(epod_match.iloc[0][col])
                    )
                    ...
                    df.at[i, col] = (
                        str(sot_match.iloc[0][col]) + "," + str(epod_match.iloc[0][col])
                    )
                elif col == "Service Remarks":
            ...
            for col in required_headers:
                if col == "Driver Team":
                    df.at[i, col] = epod_match.iloc[0]["Team"]
                elif col == "Service Name":
                    df.at[i, col] = (
                        str(nvbc_match.iloc[0][col])
                        + ","
...
def data_fill(df):
    for i in df.index:
        ...
        if (
            ...
            not sot_match.empty and not nvbc_match.empty and epod_match.empty
        ):  # only sot
            for col in required_headers:
                if col == "Driver Team":
                    df.at[i, col] = sot_match.iloc[0]["Team"]
            ...
            not sot_match.empty and nvbc_match.empty and not epod_match.empty
        ):  # only epod
            for col in required_headers:
                if col == "Driver Team":
                    df.at[i, col] = sot_match.iloc[0]["Team"]
                    ...
                    pass
        elif not sot_match.empty and nvbc_match.empty and epod_match.empty:  # only nvbc
            for col in required_headers:
                if col == "Driver Team":
                    df.at[i, col] = sot_match.iloc[0]["Team"]
                elif col == "Service Name":
...
df_sot_compiled.style.applymap(highlight, subset=["EPOD", "SOT", "NVBC"]).to_excel(
